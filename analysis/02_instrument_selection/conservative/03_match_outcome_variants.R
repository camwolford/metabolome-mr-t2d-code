...
match_proxy <- function(data, index, outcome_data, outcome_prefix, eur_bfile) {
  proxies <- gwasvcf::get_ld_proxies(
    ...
    tag_kb = 5000, tag_nsnp = 5000, tag_r2 = 0.9, threads = 1, out = tempfile()
  )
  ...
  proxies <- proxies[proxies$R >= 0.9, , drop = FALSE]
...
input_dir <- file.path(paths[["work_dir"]], "02_instrument_selection", "conservative", "Clumped_IVs")
output_dir <- file.path(paths[["work_dir"]], "02_instrument_selection", "conservative", "Matched_IVs")
...
for (metabolite_file in metabolite_files) {
  ...
  metabolite_name <- sub("_Clumped_IVs.*$", "", basename(metabolite_file))
  for (outcome_name in names(outcomes)) {
    matched <- match_outcome(metabolite_data, outcomes[[outcome_name]], outcome_name, eur_bfile)
    if (!nrow(matched)) next
    suffix <- c(t2dm = "T2DM_Matched_IVs", fg = "FG_Matched_IVs", hba1c = "HBA1C_Matched_IVs")[[outcome_name]]
