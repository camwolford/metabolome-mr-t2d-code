...
input_dir <- file.path(paths[["work_dir"]], "02_instrument_selection", "liberal", "Matched_IVs")
output_root <- file.path(paths[["output_dir"]], "02_instrument_selection", "liberal")
if (!dir.exists(input_dir)) stop(sprintf("Matched instrument directory is missing: %s", input_dir), call. = FALSE)
for (outcome in c("T2DM", "FG", "HBA1C")) dir.create(file.path(output_root, paste0("Harmonised_", outcome, "_IVs_Liberal")), recursive = TRUE, showWarnings = FALSE)
...
for (outcome in names(outcomes)) {
  metabolite_files <- list.files(input_dir, pattern = paste0(outcome, "_Matched_IVs_Liberal\\.tsv$"), full.names = TRUE)
  for (metabolite_file in metabolite_files) {
    ...
    metabolite_name <- sub(paste0(outcome, "_Matched_IVs_Liberal.*$"), "", basename(metabolite_file))
    output_file <- file.path(output_root, paste0("Harmonised_", outcome, "_IVs_Liberal"), paste0(metabolite_name, outcome, "_Harmonised_IVs_Liberal.tsv"))
