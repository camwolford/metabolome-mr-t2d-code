...
input_dir <- file.path(paths[["work_dir"]], "02_instrument_selection", "liberal", "Filtered_IVs")
output_dir <- file.path(paths[["work_dir"]], "02_instrument_selection", "liberal", "Clumped_IVs")
...
for (metabolite_file in metabolite_files) {
  ...
  if (nrow(metabolite_data) > 1) {
    ...
    metabolite_data <- ieugwasr::ld_clump_local(
      clump_input, clump_kb = 10000, clump_r2 = 0.01, clump_p = 1,
  ...
  if (!nrow(metabolite_data)) next
  metabolite_name <- sub("_IVs_Liberal.*$", "", basename(metabolite_file))
  output_file <- file.path(output_dir, paste0(metabolite_name, "_Clumped_IVs_Liberal.tsv"))
