...
harmonised_root <- file.path(paths[["output_dir"]], "02_instrument_selection", "conservative")
output_dir <- file.path(paths[["output_dir"]], "03_forward_mr", "conservative")
...
t2dm_results <- run_outcome_mr(
  file.path(harmonised_root, "Harmonised_T2DM_IVs"),
  "t2dm_Beta",
  "t2dm_SE",
  file.path(output_dir, "T2DM_MR_Results.tsv")
)
fg_results <- run_outcome_mr(
  file.path(harmonised_root, "Harmonised_FG_IVs"),
  "fg_Beta",
  "fg_SE",
  file.path(output_dir, "FG_MR_Results.tsv")
)
hba1c_results <- run_outcome_mr(
  file.path(harmonised_root, "Harmonised_HBA1C_IVs"),
  "hba1c_Beta",
  "hba1c_SE",
  file.path(output_dir, "HBA1C_MR_Results.tsv")
)
merge_outcome_results(
  t2dm_results,
  fg_results,
  hba1c_results,
  file.path(output_dir, "Full_MR_Results.tsv")
)
