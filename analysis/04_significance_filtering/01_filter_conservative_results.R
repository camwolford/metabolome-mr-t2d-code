...
filter_outcome <- function(input_file, output_file, label) {
  require_file(input_file, sprintf("Conservative %s forward-MR summary", label))
  results <- readr::read_tsv(input_file, show_col_types = FALSE)
  require_nonempty(results, sprintf("Conservative %s forward-MR summary", label))
  required_columns <- c(
  ...
  require_columns(results, required_columns, sprintf("Conservative %s forward-MR summary", label))
  require_numeric_columns(results, setdiff(required_columns, "Metabolite"), sprintf("Conservative %s forward-MR summary", label))
  require_metabolites(results, sprintf("Conservative %s forward-MR summary", label))
  ...
  require_output(output_file, sprintf("Conservative %s significant-result summary", label))
}
